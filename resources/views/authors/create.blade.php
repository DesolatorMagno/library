...
@section('content')
    <div class="w-2/3 bg-gray-200 mx-auto p-6 shadow">
        <form action="{{ route('authors.store') }}" method="post" class="flex flex-col items-center">
            ...
            <div class="pt-4">
                <input type="text" name="name" id="name" placeholder="Full Name" class="rounded px-4 py-2 w-64">
                @error('name') <p class="text-red-600">{{ $message }}</p> @enderror
            </div>
            <div class="pt-4">
                <input type="text" name="dob" id="dob" placeholder="Date of Birth" class="rounded px-4 py-2 w-64">
